...
function generateCurve(flourProtein, yeastType, tempC, hours) {
  ...
  return { points, peakTime: +peakTime.toFixed(1), peakHeight: +peakHeight.toFixed(2), lag: +lag.toFixed(1) };
}

/* ─── Sensor simulation ─────────────────────────────────────── */
const SENSOR_HISTORY = 60; // ticks stored

function initSensor(base, noise, unit, label, lo, hi, color) {
  return { base, noise, unit, label, lo, hi, color, history: [] };
}

function tickSensors(sensors) {
  return sensors.map(s => {
    const drift   = (Math.random() - 0.49) * s.noise;
    const newVal  = Math.max(s.lo - 2, Math.min(s.hi + 2, (s.history.length ? s.history[s.history.length - 1].v : s.base) + drift));
...
const INITIAL_SENSORS = [
  ...
  initSensor(415, 0.9,  "ppm",  "CO₂",          380, 600,"#F5A55A"),
  initSensor(7.2, 0.03, "pH",   "pH",            6.5, 8.0,"#C55AF5"),
];
...
const tabBtn = (active) => ({
  ...
  border: "none", borderBottom: active ? `1px solid #C8F55A` : "1px solid transparent",
...
const CurveTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: "rgba(10,10,10,0.95)", border: `1px solid ${BORD}`, padding: "8px 12px", borderRadius: 4, fontFamily: MONO, fontSize: 11, color: "#ccc" }}>
      ...
      <div>activity <span style={{ color: "#C8F55A", marginLeft: 8 }}>{(d.activity * 100).toFixed(0)}%</span></div>
    </div>
  );
};
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ position: "relative", height: 2, background: "#1a1a1a", borderRadius: 1 }}>
        <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: `${pct}%`, background: "#C8F55A", borderRadius: 1 }} />
        <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(parseFloat(e.target.value))}
          style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0, cursor: "ew-resize", margin: 0 }} />
        <div style={{ position: "absolute", top: "50%", transform: "translateY(-50%)", left: `calc(${pct}% - 5px)`, width: 10, height: 10, borderRadius: "50%", background: "#C8F55A", pointerEvents: "none" }} />
      </div>
    </div>
  );
}
...
export default function FermentationLab() {
  ...
  const handleChartMove = useCallback(e => { if (e?.activeLabel != null) setScrubX(e.activeLabel); }, []);
      ...
      <div style={{ display: "flex", alignItems: "center", borderBottom: `1px solid ${BORD}`, padding: "0 24px", gap: 4, flexShrink: 0 }}>
        ...
        {tab === "sensors" && (
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button onClick={() => setPaused(p => !p)} style={{ fontFamily: MONO, fontSize: 9, letterSpacing: "0.1em", background: "transparent", border: `1px solid ${BORD}`, color: paused ? "#C8F55A" : MUTED, borderRadius: 3, padding: "4px 10px", cursor: "pointer" }}>
              {paused ? "RESUME" : "PAUSE"}
            </button>
          </div>
        )}
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "260px 1fr", overflow: "hidden" }}>
          ...
          <div style={{ padding: "36px 24px", borderRight: `1px solid ${BORD}`, display: "flex", flexDirection: "column", overflowY: "auto" }}>
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {YEAST_TYPES.map(y => (
                  <button key={y.id} onClick={() => setYeastType(y.id)} style={{ background: yeastType === y.id ? "rgba(200,245,90,0.07)" : "transparent", border: `1px solid ${yeastType === y.id ? "rgba(200,245,90,0.25)" : "#181818"}`, borderRadius: 3, padding: "7px 10px", cursor: "pointer", textAlign: "left", fontFamily: MONO, fontSize: 11, color: yeastType === y.id ? "#C8F55A" : "#333", transition: "all 0.12s" }}>
                    {y.label}
                  </button>
                ))}
              </div>
            </div>
            <Slider label="Flour Protein" value={flourProtein} min={8} max={16} step={0.1} onChange={setFlourProtein} display={`${flourProtein.toFixed(1)}%`} />
            <Slider label="Temperature"   value={tempC}        min={4} max={38} step={0.5} onChange={setTempC}        display={`${tempC}°C / ${tempF}°F`} />
            <Slider label="Time Window"   value={hours}        min={6} max={72} step={2}   onChange={setHours}        display={`${hours}h`} />
          ...
          <div style={{ position: "relative", display: "flex", flexDirection: "column", padding: "36px 32px 36px 12px" }}>
            <div style={{ position: "absolute", top: 36, right: 32, fontFamily: MONO, textAlign: "right", opacity: scrubPoint ? 1 : 0, transition: "opacity 0.15s" }}>
              <div style={{ fontSize: 36, color: "#C8F55A", fontWeight: 300, letterSpacing: "-0.02em", lineHeight: 1 }}>
                ...
                <LineChart data={points} onMouseMove={handleChartMove} onMouseLeave={() => setScrubX(null)} margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
                  <defs>
                    <linearGradient id="lineGrad" x1="0" y1="0" x2="1" y2="0">
                      <stop offset="0%"   stopColor="#C8F55A" stopOpacity={0.25} />
                      <stop offset="40%"  stopColor="#C8F55A" stopOpacity={1} />
                      <stop offset="100%" stopColor="#C8F55A" stopOpacity={0.35} />
                    </linearGradient>
                  </defs>
                  ...
                  <Tooltip content={<CurveTooltip />} cursor={{ stroke: "rgba(255,255,255,0.04)", strokeWidth: 1 }} />
                  <ReferenceLine x={lag}      stroke="#1a1a1a" strokeDasharray="3 3" />
                  <ReferenceLine x={peakTime} stroke="rgba(200,245,90,0.1)" strokeDasharray="2 4" />
                  {scrubX !== null && <ReferenceLine x={scrubX} stroke="rgba(200,245,90,0.18)" strokeWidth={1} />}
                  <Line type="monotone" dataKey="activity" stroke="url(#lineGrad)" strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: "#C8F55A", strokeWidth: 0 }} animationDuration={500} animationEasing="ease-out" />
                </LineChart>
              </ResponsiveContainer>
            </div>
